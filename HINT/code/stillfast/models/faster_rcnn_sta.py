...
from stillfast.models.vit import load_vit
...
class RoiHeadsSTAv2(RoiHeadsSTA):
    ...
    def from_config(cls, cfg):
        ...
        #ttc_predictor = nn.Sequential(nn.Linear(representation_size+1024, 1), nn.Softplus())
        options.update({
            'verb_predictor': verb_predictor,
            # 'ttc_predictor': ttc_predictor,
    ...
    def forward(
        ...
        if targets is not None and "temporal_text_feature" in targets[0]:
            text_feas = [tar["temporal_text_feature"] for tar in targets]
            seq_mask = [tar["seq_mask"] for tar in targets]
            # timestamps = [tar["uid_time"] for tar in targets]
            max_len = 400
            text_feas = [x[-max_len:] for x in text_feas]  
            seq_mask = [x[-max_len:] for x in seq_mask]
            verb_logits_add, noun_logits_add,verb_cls_feature, noun_cls_feature = self.vit(text_feas, seq_mask)
            # llm_pred = [tar["llm_pred"] for tar in targets]
        ...
        if self.fusion == 'sum':
            ...
            global_features = global_features.view(global_features.shape[0], global_features.shape[1], -1).mean(-1)
            BS = global_features.shape[0]
            proposal_nums = [proposal.shape[0] for proposal in proposals]
            max_proposals = max(proposal_nums)
            gl_box_features = torch.split(box_features, proposal_nums, dim=0)  # [BxNxC]
            _gl_box_features = []
            _gl_box_masks = []
            for num, box_feature in zip(proposal_nums, gl_box_features):
                mask = torch.ones((max_proposals), device=global_features.device)
                box_feature = box_feature.reshape(1, -1, box_feature.shape[-1])
                if num < max_proposals:
                    padding = max_proposals - num
                    mask[num:] = 0.
                    padding_feat = torch.zeros((1, padding, box_feature.shape[-1]), device=global_features.device)  # Bx1xC -> BxpaddingxC
                    box_feature = torch.cat([box_feature, padding_feat], dim=1)
                _gl_box_features.append(box_feature)
                _gl_box_masks.append(mask)
            _gl_box_masks = torch.cat(_gl_box_masks)  # BxMax_Proposal

            # gl_box_features = box_features.reshape(BS, -1, box_features.shape[-1]) # [BS*NUM_PROPOSALS, 1024] -> [BS, NUM_PROPOSALS, 1024]
            gl_box_features = torch.cat(_gl_box_features)
            ...
            # global_features_verb = verb_cls_feature.unsqueeze(1).expand(BS, gl_box_features.shape[1], verb_cls_feature.shape[-1])

            # global_features_ttc = verb_cls_feature_TTC.unsqueeze(1).expand(BS, gl_box_features.shape[1], verb_cls_feature_TTC.shape[-1])

            gl_box_features_back = gl_box_features

            gl_box_features = torch.cat([gl_box_features, global_features], dim=-1)  # [BS, NUM_PROPOSALS, 1024+256]

            # gl_box_features_verb = torch.cat([gl_box_features_back,global_features_verb], dim=-1)

            # gl_box_features_ttc = torch.cat([gl_box_features_back,global_features_ttc], dim=-1)
            ...
            # gl_box_features_verb = self.mapper1(gl_box_features_verb).reshape(-1, box_features.shape[-1])
            # gl_box_features_ttc = self.mapper2(gl_box_features_ttc).reshape(-1, box_features.shape[-1])

            gl_box_features = gl_box_features[_gl_box_masks != 0.]
            # gl_box_features_verb = gl_box_features_verb[_gl_box_masks != 0.]
            # gl_box_features_ttc = gl_box_features_ttc[_gl_box_masks != 0.]


            if self.residual:
                gl_box_features = box_features + gl_box_features
                # gl_box_features_verb = box_features + gl_box_features_verb
                # gl_box_features_ttc = box_features + gl_box_features_ttc
        else:
        ...
        if self.training:
            ...
            valid_targets = [None]*num_images 
            for img_id in range(num_images): 
                idx = matched_idxs[img_id] 
                vl = targets[img_id]['verb_labels']
                tt = targets[img_id]['ttc_targets'] 
                verb_labels[img_id] = vl[idx] 
                ttc_targets[img_id] = tt[idx] 
                valid_targets[img_id] = noun_labels[img_id] != 0 
            verb_labels = torch.cat(verb_labels, dim=0)
            ttc_targets = torch.cat(ttc_targets, dim=0).unsqueeze(-1)
            valid_targets = torch.cat(valid_targets, dim=0)
    ...
    def postprocess_detections_sta(
        ...
        device = noun_logits.device
        noun_classes = noun_logits.shape[-1] 
        verb_classes = verb_logits.shape[-1] 
