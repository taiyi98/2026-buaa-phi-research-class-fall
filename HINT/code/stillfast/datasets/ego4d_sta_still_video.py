...
class Ego4dShortTermAnticipationStillVideo(Ego4dShortTermAnticipationStill):
    ...
    def __init__(self, cfg, split):
        ...
        self.uid_fea_dir = "/data/zst/ganov2/new_uid_nv_features_4_21"
        self.uid_time_dir = "/data/stillfast/uid_time" 
        # self.llm_pred_path = "/data/zst/ganov2/llm_sta_721.json"
        # with open(self.llm_pred_path,"r") as f:
    ...
    def __getitem__(self, idx):
        ...
        #### add memory data
        # uid_time = torch.load(os.path.join(self.uid_time_dir,f"{uid}.pth"), map_location=torch.device('cpu'))
        fea_path = os.path.join(self.uid_fea_dir,video_id,f"{uid}.pth") 
        fea = torch.load(fea_path, map_location=torch.device('cpu'))
        temporal_text_feature, graph_mask, seq_mask= fea["text_feas"], fea["graph_mask"], fea["sequence_mask"]
        extral_data = {
            "temporal_text_feature":temporal_text_feature,
            "graph_mask":graph_mask,
            "seq_mask":seq_mask,
            # "llm_pred": self.llm_data.get(uid,0)
            # "uid_time":uid_time
        }

        return {'still_img': still_img, 'fast_imgs': fast_imgs, 'targets': targets, 'uids': uid, "extral_data":extral_data}