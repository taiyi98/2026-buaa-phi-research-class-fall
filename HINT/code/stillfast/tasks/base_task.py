...
class BaseTask(LightningModule):
    ...
    def load_from_checkpoint_list(self, ckps):
        ...
        print("loading the whole model ckpt")
        # head_dict = torch.load("/data/zst/ganov2/checkpoints_gat_simple/best_acc_model.pt")
        # for k,v in head_dict.items():
        #     k = "model.roi_heads.vit."+k
        #     state_dict[k] = v
        ...
        for k, v in state_dict.items():
            ...
            if name in model_dict and model_dict[name].shape != v.shape:
                # change v to be the same shape as model_dict[k],init it
                print("skip loading {}, required shape{}, loaded shape{}".format(name, model_dict[name].shape, v.shape))
                vv = torch.randn(model_dict[name].shape)
                state_dict[k] = vv
